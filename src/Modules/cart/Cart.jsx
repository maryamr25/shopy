...
function Cart() {
    ...
    // if(!carts.length) <div>Carts is Empty</div>
    if (!carts.length) {
        return <div className="text-center mt-10 text-xl font-semibold">Cart is Empty</div>;
      }


// incearse the quantity
    const  handleInc = (id) => {
    ...
    // total value
  useEffect(()=>{
    const totalValue = carts.reduce((acc , item) => acc + item.price * item.quantity,0)
    setTotal(totalValue)
  ...
  return (
    ...
    <div className="container mx-auto mt-10">
      <div className="flex shadow-md my-10">
                ...
                return (
                  ...
                  <div className="flex items-center hover:bg-gray-100 -mx-8 px-6 py-5">
                    <div className="flex w-2/5"> 
                      <div className="w-20">
                        <img className="h-24 w-full object-contain" src={cart?.image} alt={cart?.title}/>
                      </div>
                      <div className="flex flex-col justify-between ml-4 flex-grow">
                        <span className="font-bold text-sm">{cart?.title}</span>
                        <span className="text-red-500 text-xs">{cart?.category}</span>
                        <div className="font-semibold hover:text-red-500  text-xs cursor-pointer" onClick={() => removeProduct(cart?.id)}>Remove</div>
                      </div>
                    </div>
                    <div className="flex justify-center w-1/5">
                      <svg className="fill-current w-3 cursor-pointer" viewBox="0 0 448 512" onClick={() => handleDec(cart?.id)}><path d="M416 208H32c-17.67 0-32 14.33-32 32v32c0 17.67 14.33 32 32 32h384c17.67 0 32-14.33 32-32v-32c0-17.67-14.33-32-32-32z"/>
                      </svg>
        ...
        <div id="summary" className="w-1/4 px-8 py-10 shadow-lg shadow-black">
          ...
          <button className="bg-orange-700 hover:bg-orange-800 rounded-full px-5 py-2 text-sm text-white uppercase">Apply</button>
          <div className="border-t mt-8">
            <div className="flex font-semibold justify-between py-6 text-sm uppercase">
              <span>Total Cost</span>
              <span>${(total + 10).toFixed(2)}</span>
            </div>
            <Link to={'/checkout'} className="bg-gradient-to-t from-primary to-secondary font-semibold  py-3 px-4 text-sm text-white uppercase w-full  rounded-full">Checkout</Link>
          </div>
        </div>
